  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(checklistItems).values([
        ...DEFAULT_CHECKLIST.required.map((label, i) => ({ workspaceId: workspace.id, checklistId: template.id, text: label, templateId: template.id, label, required: true, sortOrder: i })),
        ...DEFAULT_CHECKLIST.optional.map((label, i) => ({ workspaceId: workspace.id, checklistId: template.id, text: label, templateId: template.id, label, required: false, sortOrder: DEFAULT_CHECKLIST.required.length + i })),
      ]);
